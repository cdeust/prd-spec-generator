...
# /generate-prd-v3 — Dispatcher Loop

You are the host. Drive the pipeline by:

1. Calling `start_pipeline_v2`.
...
Example: `/generate-prd-v3 /Users/me/projects/myapp build OAuth login` →
- `codebase_path = "/Users/me/projects/myapp"`
- `feature_description = "build OAuth login"`

Example: `/generate-prd-v3 build OAuth login` →
...
- **Never** call `start_pipeline_v2` / `submit_action_result` / `get_pipeline_state_v2` from inside an Agent tool call (i.e., from inside a spawned subagent).
...
- If the message says "unknown run_id", the MCP server restarted and lost state. Begin a new run with `start_pipeline_v2`.
...
```
get_pipeline_state_v2({ run_id, format: "full" })
```

The `errors[]` field in the returned state contains the full diagnostic trail.

`get_pipeline_state_v2` is **read-only** and does not advance the pipeline. Use it for diagnosis only.

---

## Notes

- This command is the v3 entry point. The legacy `/generate-prd` command remains for backwards compatibility (older prose workflow).
